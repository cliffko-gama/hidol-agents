...
#### ⚠️ 主題一致性檢查（Theme Spine，新增核心項目）
這是最常導致文章「沒有焦點」的問題：

1. **核心角度唯一性**：全文是否圍繞**一個**核心情感/觀點展開？
   - ✅ 每個 section 都在推進同一個主題
   - ❌ 不同 section 在講不相關的事，湊在一起像「Moment 集錦」而不是「Feature Story」

2. **Intro 定調**：開場的前 2 句有沒有清楚點出這篇文章的核心角度（Theme Spine）？
   - 讀完 intro，讀者應該知道「這篇文章想說的是什麼」
   - ❌ 如果 intro 只是模糊地「描述情境」而沒有指向核心角度，就是 fail

3. **Conclusion 呼應**：結語的互動問題是否扣回 Theme Spine？
   - ✅ 互動問題讓讀者反思與 Theme Spine 相關的個人經驗
   - ❌ 互動問題太泛（「你怎麼看？」「你有什麼感想？」）

4. **社群洞察整合**：如果有引用 Threads/IG/Dcard 的社群觀察，
   是否自然融入文章而非硬貼？

**扣分標準**：
- 結構混亂/跳躍：扣 3-4 分
- 全文缺乏一個貫穿的核心角度（Theme Spine 不清楚）：扣 3-4 分
- Intro 前 2 句沒有指向核心角度：扣 2-3 分
- 虎頭蛇尾：扣 2-3 分
- 段落過長（手機上超過 3 行）：扣 1-2 分
- 結語互動問題太泛，未扣回 Theme Spine：扣 1-2 分
...
#### Gen Z Mobile-First 格式檢查
Z 世代主要在手機上讀文章，格式是可讀性的核心：

1. **段落長度**：每段是否在手機上不超過 3 行？
   - 手機 3 行 ≈ 桌機 1.5 行 ≈ 50-80 字
   - ❌ 如果有段落在手機上需要滑動才能看完，一律標記

2. **Hook-First**：每個 section 的前 2 句是否夠吸引人？
   - 重點要放在前面，不要藏在段落中間

3. **Scroll-Stopper**：全篇是否有至少 2 個讓人想停下來的「錨點」？
   - 一句超有感的 Moment 直接引用
   - 一個讓人想截圖的金句
   - 一個意外的數字或對比
...
- 大量語病：扣 3-4 分
- 用詞生硬/過於文青：扣 2-3 分
- 閱讀節奏拖沓（大段落、重點埋在後面）：扣 2-3 分
- 段落過長（手機 3 行以上）：扣 1-2 分
- 全篇沒有 Scroll-Stopper 時刻：扣 1-2 分
...
1. **先點出 Theme Spine 問題（如果有）**：
   - 全文有沒有一個清楚的核心角度？如果沒有，直接告訴撰寫者「這篇文章缺乏 Theme Spine，建議以 [XXX] 作為核心情感重寫 intro 和 conclusion」
2. **具體**：指出哪個段落、哪句話有問題
3. **有方向**：不只說「這裡不好」，要說「建議改成什麼方向」
4. **有優先級**：先列最嚴重的問題（Theme Spine > 事實錯誤 > 調性 > 格式）
5. **可操作**：撰寫者看完就知道要改什麼

範例：
✅ 「全文缺乏 Theme Spine。intro 只說了『這篇文章記錄 FEniX 見面會』，但沒有告訴讀者這篇文章的核心情感是什麼。建議以 moment_trend_signals 的 trending_factor 重寫 intro，前 2 句就要點出『等待的值得感』這個核心角度。」
